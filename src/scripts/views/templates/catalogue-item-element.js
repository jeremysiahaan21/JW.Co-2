...
class CatalogueItemElement extends LitElement {
  static properties = {
    data: { type: Object },
  };
  ...
  * {
  ...
  <article class="catalogue-list-item" id="${data.id}" tabindex="0">
    <div class="catalogue-thumbnail" style="background-image: url(${CONFIG.BASE_IMAGE_URL.large}/${data.pictureId});">
    ...
    <div class="catalogue-content">
        <h2 class="catalogue-content__title"><a href="/#/detail/${data.id}">${data.name}</a></h2>
        <p class="catalogue-content__description">${data.description}</p>
    </div>
  </article>`;
}

customElements.define('catalogue-item', CatalogueItemElement);

export default CatalogueItemElement;
